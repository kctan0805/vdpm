...
int main( int argc, char **argv )
{
    ...
    bool do_srmesh_conv = false;
    unsigned int faceTarget = 0;
    if (arguments.read("--vdpm", faceTarget))
    {
        do_srmesh_conv = true;
    }
    ...
    if ( root.valid() )
    {
        ...
        if (do_srmesh_conv)
            optimizer.optimize(root.get(), osgUtil::Optimizer::DEFAULT_OPTIMIZATIONS | osgUtil::Optimizer::INDEX_MESH);
        else
        ...
        if (do_srmesh_conv)
        {
            SRMeshConverter conv(faceTarget, fileNameOut);
            root->accept(conv);
        }
