...
bool MxPropSlim::decimate(uint target)
{
	...
	if( m->vertex_is_valid(v1) && m->vertex_is_valid(v2) )
	{
		...
		pack_to_vector(v1, conx.vec1);
		pack_to_vector(v2, conx.vec2);
...
void MxPropSlim::update_pre_expand(const MxPairContraction&)
{
}

void MxPropSlim::update_post_expand(const MxPairContraction& conx)
{
    MxVertexID v1=conx.v1, v2=conx.v2;
    uint i;
...
void MxPropSlim::apply_expansion(const MxPairContraction& conx)
{
	...
	MxPropPairContraction &prop_conx = (MxPropPairContraction &)conx;
	unpack_from_vector(conx.v1, prop_conx.vec1);
	unpack_from_vector(conx.v2, prop_conx.vec2);
}
