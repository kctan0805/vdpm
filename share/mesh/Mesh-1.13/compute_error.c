...
static void realloc_triag_sample_error(struct triag_sample_error *tse, int n)
{
  ...
  if (tse->n_samples_tot >  tse->buf_sz) {
	...
	tse->max_dist_vecs = xa_realloc(tse->max_dist_vecs,
		tse->n_samples_tot*sizeof(dvertex_t));
  }
  if (n != 0) {
...
static void free_triag_sample_error(struct triag_sample_error *tse)
{
  ...
  tse->err = NULL;
  tse->err_lin = NULL;
  free(tse->max_dist_vecs);
  tse->max_dist_vecs = NULL;
}
                             ...
                             struct misc_stats *m_stats)
{
  ...
  for (i=0; i<tse->n_samples_tot; i++) {
	...
	if (err_max < err_local) { err_max = err_local; dss_stats->max_dist_vec = tse->max_dist_vecs[i]; }
  }
                   ...
                   dvertex_t bbox_min)
{
  ...
  for (i=0, imax=tl->n_triangles; i<imax;i++) {
     ...
     * vertices in tl). */
    m_a = (int)(unsigned int)((tl->triangles[i].a.x-bbox_min.x)/cell_sz);
    n_a = (int)(unsigned int)((tl->triangles[i].a.y - bbox_min.y) / cell_sz);
    o_a = (int)(unsigned int)((tl->triangles[i].a.z - bbox_min.z) / cell_sz);
    m_b = (int)(unsigned int)((tl->triangles[i].b.x - bbox_min.x) / cell_sz);
    n_b = (int)(unsigned int)((tl->triangles[i].b.y - bbox_min.y) / cell_sz);
    o_b = (int)(unsigned int)((tl->triangles[i].b.z - bbox_min.z) / cell_sz);
    m_c = (int)(unsigned int)((tl->triangles[i].c.x - bbox_min.x) / cell_sz);
    n_c = (int)(unsigned int)((tl->triangles[i].c.y - bbox_min.y) / cell_sz);
    o_c = (int)(unsigned int)((tl->triangles[i].c.z - bbox_min.z) / cell_sz);
...
static double dist_pt_surf(dvertex_t p, const struct triangle_list *tl,
                           ...
                           int **dcl_buf, int *dcl_buf_sz, dvertex_t *pv)
{
  ...
  do {
    ...
    for (cur_cell = dcl[cell_idx].list[k].cell,
      ...
      do { /* cell has always one triangle at least, so do loop is OK */
        ...
        stats->n_triag_scans++;
#endif
        dist_sqr = dist_sqr_pt_triag(&triags[t_idx],&p,&q);
  ...
  for (k=0, kmax=m1->num_faces; k<kmax; k++) {
    ...
    for (i=0; i<tse.n_samples_tot; i++) {
      tse.err_lin[i] = dist_pt_surf(ts.sample[i],tl2,fic,
                                    ...
                                    &prev_p,prev_d,&dcl_buf,&dcl_buf_sz,&tse.max_dist_vecs[i]);
      prev_p = ts.sample[i];
